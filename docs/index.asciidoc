...
If specified, prefix all memcached keys with the given string; this is useful if all keys being used by this plugin share a common prefix.

[id="plugins-{type}s-{plugin}-get"]
===== `get`

  * Value type is <<map,map>>
  * There is no default value for this setting.
